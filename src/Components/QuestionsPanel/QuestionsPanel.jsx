...
function QuestionsPanel({changeCurrentScreen}){
    const dispatch = useDispatch();

    let questions = useSelector(state => state.questions.uncompletedQuestions);
            ...
            <>
                <div className="h-[10%] flex items-center justify-between relative max-[600px]:mb-2">
                    <span className="text-xl font-semibold opacity-70">
                        {categoriesObj[questions[currentQuestion].category]} - {difficultyObj[questions[currentQuestion].difficulty]}
                    </span>
                    <div className="text-xl font-bold flex items-center gap-3 bg-blue-300 px-3 py-2 rounded max-[600px]:p-1">
                        <span>Time Left</span>
                        <span className="bg-blue-600 text-blue-200 px-2 rounded w-12 text-center">{timer}</span>
                    </div>
                ...
                <div className="h-[75%] flex flex-col justify-between gap-1">
                    ...
                    <div className="flex flex-col justify-evenly gap-4" onClick={(e)=>{
                        if(e.target.tagName !== 'BUTTON' || !isDisabled) return null
                        clearInterval(timerId);
                        if(e.target.textContent === questions[currentQuestion].correctAnswer){
                            ...
                            ref.current.classList.add('correctAnswer');
                            setIsCorrect(false);
                        }
                        setIsDisabled(false);
                    }}>
                ...
                <div className="h-[15%] flex justify-between items-center">
                    ...
                    <button disabled={isDisabled} className="btn" onClick={()=>{
                        clearInterval(timerId)
                        refLine.current.style.width = `${((currentQuestion+1)/questions.length)*100}%`;
