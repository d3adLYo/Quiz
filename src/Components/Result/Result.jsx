...
function Result({changeCurrentScreen}){
    ...
    result.forEach((elem)=>{
        if(elem.category in categoriesResultObj){
            ...
            categoriesResultObj[elem.category] = {total: 1, correct: 0};
            categoriesResultObj[elem.category].name = categoriesObj[elem.category];
            if(elem.isCorrect) categoriesResultObj[elem.category].correct += 1;
        }
    });

    let categoriesResultArray = [];
    ...
    result.forEach((elem)=>{
        if(elem.category in profileStatsObj){
            if(elem.isCorrect) profileStatsObj[elem.category][elem.difficulty].correct += 1;
            profileStatsObj[elem.category][elem.difficulty].total += 1;
        }
        else{
        ...
        <>
            <div className='wrapper'>
                ...
                <div className="h-[25%] text-4xl font-bold flex flex-col items-center justify-center max-[600px]:mb-4">
                    ...
                    <p className="rounded-lg bg-yellow-400 w-32 h-20 flex items-center justify-center">{easy.correct + medium.correct + hard.correct} / {result.length}</p>
                </div>
                <button className="btn self-center" onClick={()=>{
